...
void train_yolo(char *cfgfile, char *weightfile)
{
    ...
    while(get_current_batch(net) < net.max_batches){
        ...
        printf("[batch_num:%d]: loss:%f, avg_loss:%f, %f rate, %lf seconds, %d images\n", i, loss, avg_loss, get_current_rate(net), sec(clock()-time), i*imgs);
        if(i%100==0){
            printf("weights is saving :%s\n", backup_directory);
            char buff[256];
...
void validate_yolo(char *cfgfile, char *weightfile)
{
    ...
    char *base = "results/test_";
    list *plist = get_paths("data/mikan_list.txt");
...
void validate_yolo_recall(char *cfgfile, char *weightfile)
{
    ...
    char *base = "results/test_recall_";
    list *plist = get_paths("data/mikan_list.txt");
    //list *plist = get_paths("data/voc.2007.test");
    char **paths = (char **)list_to_array(plist);
...
void test_yolo(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(1){
        ...
        time=clock();
        float *predictions = network_predict(net, X);
        printf("%s: Predicted in %f seconds. detected objects:%d \n", input, sec(clock()-time), sizeof(predictions) );
        convert_yolo_detections(predictions, l.classes, l.n, l.sqrt, l.side, 1, 1, thresh, probs, boxes, 0);
        if (nms) do_nms_sort(boxes, probs, l.side*l.side*l.n, l.classes, nms);
        //draw_detections(im, l.side*l.side*l.n, thresh, boxes, probs, voc_names, NULL, CLASSNUM);
